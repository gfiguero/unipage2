...
namespace Uni\AdminGeneratorBundle\Command;
...
use Uni\AdminGeneratorBundle\Generator\DoctrineCrudGenerator;
use Uni\AdminGeneratorBundle\Generator\DoctrineFormGenerator;
...
class GenerateDoctrineCrudCommand extends GenerateDoctrineCommand
{
    ...
    {
        $this
            ->setName('uni:admingenerate:crud')
...
EOT
    ...
    {
        $adminBundle = $this->getContainer()->get('kernel')->getBundle('UniAdminBundle');
        ...
        if (!$ret) {
            $help = sprintf("        <comment>resource: \"@UniAdminBundle/Resources/config/routing/%s.%s\"</comment>\n", strtolower(str_replace('\\', '_', $entity)), $format);
